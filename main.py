...
def brutexss():
	...
	print banner
	def again():
		inp = raw_input(" --> [e/E]xit or launch [a/A]gain? ").lower()
		if inp == 'a':
			brutexss()
		elif inp == 'e':
			exit()
		...
		try:
			try:
				try:
					...
					wordlist = raw_input(" --> Enter location of Wordlist (Press Enter to use default wordlist)\n ")
					if len(wordlist) == 0:
						wordlist = 'wordlist.txt'
	...
	try:
		methodselect = raw_input(" --> Select method: [g/G]ET or [p/P]OST : ").lower()
		if methodselect == 'g':
			GET()
		elif methodselect == 'p':
			POST()
